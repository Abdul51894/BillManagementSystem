...
<body>
	<div class="container">
		...
		<h2 class="mb-4">Bill Dashboard</h2>
		...
		<table class="table table-bordered table-hover">
			...
			<tbody>
				...
				try {
					...
					// 👇 Updated query to include only bill_number (not bill_id)
					String sql = "SELECT bill.bill_number, party.party_name, bill.total_amount, bill.bill_date "
					+ "FROM bill INNER JOIN party ON bill.party_id = party.party_id"; // 👈 UPDATED SQL
				...
				<tr>
					...
					<td>
						<!-- 👇 Updated: Pass bill_number in query string instead of bill_id -->
						<a
						href="viewBill.jsp?bill_number=<%=rs.getString("bill_number")%>"
						class="btn btn-info btn-sm">View</a> <a
						href="updateBill.jsp?bill_number=<%=rs.getString("bill_number")%>"
						class="btn btn-warning btn-sm">Update</a> <a
						href="deleteBill.jsp?bill_number=<%=rs.getString("bill_number")%>"
